...
args.config_grid_def = {'type': 'p_left', 'spacing': 'list', 'list': [0.1, 0.3, 0.5]}
...
def run_main(args, save_result=True, load_run_data_flag=False, result_dir_to_load='', save_PDF=False, plot=True):
    ...
    if plot:
        ...
        mixing_time_labels = ['Slow mixing', 'Moderate mixing', 'Fast mixing']
        for i_config in range(args.n_configs):  # grid of number of trajectories to generate
            # if i_config in [1,3]:
            #     continue
            if args.config_grid_def['type'] == 'p_left':
                args.mrp_def['p_left'] = config_grid[i_config]
                M = MRP(args)
                mixing_time = np.around(calc_mixing_time(M.P), decimals=3)
                # label = r'$p_l=${} , $\beta=${}'.format(config_grid[i_config], mixing_time)
                label = mixing_time_labels[i_config]
                print(label + r': $p_l=${} , $\beta=${}'.format(config_grid[i_config], mixing_time))
            else:
                label = '{}'.format(config_grid[i_config])

            plt.errorbar(alg_param_grid, planing_loss_avg[i_config], yerr=planing_loss_std[i_config] * ci_factor,
                         marker='.', label=label)
            # plt.plot(alg_param_grid,  planing_loss_avg[i_config], label=label)
            # plt.fill_between(alg_param_grid,  planing_loss_avg[i_config] - planing_loss_std[i_config] * ci_factor,
            #                  planing_loss_avg[i_config] + planing_loss_std[i_config] * ci_factor, alpha=0.2)

            # Mark the best gamma:
            i_best = np.argmin(planing_loss_avg[i_config])
            plt.scatter(alg_param_grid[i_best], planing_loss_avg[i_config][i_best], marker='*', s=400)
        plt.grid(True)
        if args.param_grid_def['type'] == 'l2_proj':
            plt.xlabel(r'$L_2$ Regularization Factor  - Projection Phase')
        elif args.param_grid_def['type'] == 'l2_fp':
            plt.xlabel(r'$L_2$ Regularization Factor  - Fixed-Point Phase')
        elif args.param_grid_def['type'] == 'gamma_guidance':
            plt.xlabel(r'Guidance Discount Factor $\gamma$')
        elif args.param_grid_def['type'] == 'l2_factor':
            plt.xlabel(r'$L_2$ Regularization Factor  - BOTH ')
